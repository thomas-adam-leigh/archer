...
> ✅ **API auth: send your Supabase JWT (ARC-83).** A client authenticates user-facing routes with `Authorization: Bearer <supabase access token>`. The API verifies the token's signature + claims (expiry, audience `authenticated`) and takes the user id from the verified `sub` (= `auth.uid()`) — identity comes from the **token, never from a caller-supplied `user`/`userId`**. A mismatching explicit `user` is refused (**403**); a missing/invalid/expired/tampered token is **401** (fail closed). Verification is local HS256 against `SUPABASE_JWT_SECRET`, falling back to Supabase `/auth/v1/user` for asymmetric-key (ES256) projects (`services/api/src/auth.ts`). The fallback's required `apikey` header is resolved from any of `SUPABASE_SECRET_KEY` / `SUPABASE_PUBLISHABLE_KEY` (the current key-env names the deployed container provides) or the legacy `SUPABASE_SERVICE_ROLE_KEY` / `SUPABASE_ANON_KEY` (ARC-87). The server-to-server `x-archer-secret` plane (`ARCHER_API_SECRET`, full trust, may act for any `user`; or `ARCHER_API_DEV_OPEN=1` in non-prod) is unchanged and still used by the CLI / cron. The internal **control-plane** routes (`/commands/*`, `/hooks/*`) accept the service secret **only** — never a user JWT. See §10.
...
User-facing routes accept **either** the user's Supabase JWT (`Authorization: Bearer …`, scoped to that user — ARC-83) **or** the `x-archer-secret` header (`ARCHER_API_SECRET`, full trust), or dev-open in non-prod. The internal control-plane routes (`/commands/*`, `/hooks/*`) take the service secret **only**. The **three owner-gated `/decide` routes** additionally require the `x-archer-admin-secret` header (`ARCHER_API_ADMIN_SECRET`) — they are flagged below. All take/return JSON. With a JWT, `user` is the verified `auth.uid()` (a mismatching explicit `user` → 403); with the service secret, `user` is the caller-supplied value, defaulting to `process.env.ARCHER_USER_ID` when omitted. Source: `services/api/src/app.ts` (now an `OpenAPIHono` app). The live, machine-readable spec is at **`GET /openapi.json`** and a self-hosted **Scalar** reference UI at **`GET /reference`** (`docs/SECURITY-OPS-RUNBOOK.md` §4); both declare the `serviceSecret`/`ownerSecret` schemes.
...
| POST | `/onboarding/revise` | `{ threadId, feedback }` | `{ threadId, runId, status:"proposed", versionId, proposalId, attributes }` — revise the candidate's **open** proposed draft from `feedback` (typed, or voice transcribed client-side) as a streamed AG-UI run (ARC-85). Amends the draft (re-using the retained résumé text or the conversation transcript) rather than rebuilding from scratch, persists a **new** proposed version, and rejects the superseded one so exactly one proposal stays open. **404** when there's no open draft. Owner resolved from the thread |
...
- The API authenticates user-facing routes with the user's **Supabase JWT** (`authenticate()` in `app.ts`; verified in `auth.ts`) and scopes data to the verified `auth.uid()` — a caller-supplied `user`/`userId` that disagrees with the token is refused (403), so a mobile client **can** call the API directly with its session token. The `x-archer-secret` plane stays for the CLI / cron (full trust). Direct **Supabase reads** under RLS remain available for read screens.
...
- Owner/human-decision routes are now gated behind a **separate admin secret** (`x-archer-admin-secret`, ARC-51) and the API **fails closed in production** if `ARCHER_API_SECRET` is unset (ARC-55). User-facing routes now **verify the Supabase JWT** and scope to the verified `auth.uid()` (ARC-83), so a client may call them directly with its session token; the prior "trusts the caller-supplied `user`" gap is closed. (`SUPABASE_JWT_SECRET` and/or `SUPABASE_URL` configure verification.)
