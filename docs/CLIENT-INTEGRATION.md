...
> ✅ **API auth: send your Supabase JWT (ARC-83).** A client authenticates user-facing routes with `Authorization: Bearer <supabase access token>`. The API verifies the token's signature + claims (expiry, audience `authenticated`) and takes the user id from the verified `sub` (= `auth.uid()`) — identity comes from the **token, never from a caller-supplied `user`/`userId`**. A mismatching explicit `user` is refused (**403**); a missing/invalid/expired/tampered token is **401** (fail closed). Verification is local HS256 against `SUPABASE_JWT_SECRET`, falling back to Supabase `/auth/v1/user` for asymmetric-key projects (`services/api/src/auth.ts`). The server-to-server `x-archer-secret` plane (`ARCHER_API_SECRET`, full trust, may act for any `user`; or `ARCHER_API_DEV_OPEN=1` in non-prod) is unchanged and still used by the CLI / cron. The internal **control-plane** routes (`/commands/*`, `/hooks/*`) accept the service secret **only** — never a user JWT. See §10.
