...
3. **Orchestration mechanism.** **[decision, 2026-06-20 — ARC-7]** Standardize on **Postgres triggers + webhooks + Hono** as the spine — implemented as **pg_cron** schedules (13:00 weekday Collect, per-minute Match) + **pg_net** webhooks from status-change/failure triggers to the Hono API (`20260620180000_event_engine.sql`). **n8n is retired** as an Archer orchestrator: it stays on the host only as build/glue plumbing (Komodo redeploys, alert fan-out), never as a second event engine, so there is one source of orchestration truth.
