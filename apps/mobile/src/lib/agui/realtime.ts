 ...
 *
 * Lynx has no `@supabase/supabase-js` (browser/Node assumptions don't hold in the
 * dual-thread runtime), so we speak the Phoenix websocket protocol Realtime uses
 * directly over the global `WebSocket`. Following `storage.ts`, the transport is an
 * injectable interface with a real implementation and a no-op fallback when no
 * `WebSocket` is present (tests under jsdom, or a host without one) so the app
 * never crashes for the want of a socket — it simply relies on history-restore +
 * the synchronous run response until live push is available.
 */
...
// globals the Lynx runtime may not declare.
...
export function createSupabaseRealtime(
  ...
  return {
    subscribe(opts: SubscribeOptions): RealtimeSubscription {
      ...
      const url = `${base}/realtime/v1/websocket?apikey=${SUPABASE_PUBLISHABLE_KEY}&vsn=1.0.0`;
...
export function resolveRealtime(): RealtimeTransport {
  const WebSocketImpl = findWebSocket();
  return WebSocketImpl ? createSupabaseRealtime(WebSocketImpl) : noopRealtime;
}
