...
// Stub the env-backed config so importing the api/realtime modules doesn't throw
// under vitest (no PUBLIC_* env). The session is driven via injected fakes.
...
describe('createThreadSession', () => {
  test('loadHistory seeds the view from the persisted log', async () => {
    ...
    expect(view.messages).toEqual([
      { id: 'r1:m1', role: 'assistant', content: 'Hi' },
    ]);
