 ...
 * review, preferences, the conversational path — Milestones 4–7). It folds one
 * thread's event log into a `ThreadView` and keeps it current from three sources,
 * reconciled by `(runId, seq)` so they converge regardless of arrival order:
 *
...
import { type RealtimeTransport, resolveRealtime } from './realtime.js';
...
/** The HTTP surface the session needs — injectable so it can be tested without
 *  a live API. Defaults bind the authenticated `api` client to this session. */
...
export interface ThreadSessionOptions {
  threadId: string;
  /** The user's Supabase access token (for the API client + Realtime RLS). */
  accessToken: string;
  /** Called with the fresh view whenever any source advances the thread. */
  onChange?(view: ThreadView): void;
  /** Live transport; defaults to Supabase Realtime (no-op when unavailable). */
  transport?: RealtimeTransport;
  /** HTTP client; defaults to the authenticated Archer API client. */
  http?: AguiHttp;
}
...
export function createThreadSession(opts: ThreadSessionOptions): ThreadSession {
  const { threadId, accessToken } = opts;
  const transport = opts.transport ?? resolveRealtime();
  const http: AguiHttp = opts.http ?? {
    get: (path) => apiGet(path, accessToken),
    post: (path, body) => apiPost(path, accessToken, body),
  };
  ...
  return {
    ...
    apply(resp) {
      const changed =
        resp.runId && resp.events
          ? log.add(fromRunResponse(resp.runId, resp.events))
          : false;
      return emit(changed);
    },
